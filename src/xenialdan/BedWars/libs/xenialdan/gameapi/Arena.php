...
namespace xenialdan\BedWars\libs\xenialdan\gameapi;
...
use xenialdan\BedWars\libs\xenialdan\gameapi\libs\xenialdan\apibossbar\DiverseBossBar;
use xenialdan\BedWars\libs\xenialdan\gameapi\event\UpdateSignsEvent;
use xenialdan\BedWars\libs\xenialdan\gameapi\event\WinEvent;
use xenialdan\BedWars\libs\xenialdan\gameapi\gamerule\BoolGameRule;
use xenialdan\BedWars\libs\xenialdan\gameapi\gamerule\GameRuleList;
use xenialdan\BedWars\libs\xenialdan\gameapi\task\StartTickerTask;
...
class Arena
{
    ...
    public function joinTeam(Player $player, string $teamname = "")
    {
        if ($this->getState() === self::SETUP) {
            $player->sendMessage(TextFormat::RED . TextFormat::BOLD . "Die Arena ist in Bear´beitung bitte warte bis Sie fertig ist.");
            return false;
        }
        if (($this->getState() === self::INGAME && count($this->getPlayers()) <= 0) || $this->getState() === self::STOP) {
            $player->sendMessage(TextFormat::RED . TextFormat::BOLD . "Dieses Spiel läuft noch");
            $player->sendMessage(TextFormat::RED . TextFormat::BOLD . "Beim Versuch das Spiel zu stoppen ist ein Fehler aufgetretten.");
        ...
        if (($this->getState() === self::INGAME) && empty($teamname) && is_null(API::getArenaOfPlayer($player))) {
            $player->sendMessage(TextFormat::RED . TextFormat::BOLD . "Dieses Spiel läuft bereits");
            return false;
        }
        ...
        if (!$team instanceof Team) {
            $player->sendMessage(TextFormat::RED . TextFormat::BOLD . "Du kannst dem Team $teamname nicht Joinen, Begründung: Kein Team existiert mit dem Namen");
            return false;
        }
        if (count($team->getPlayers()) >= $team->getMaxPlayers()) {
            $player->sendMessage(TextFormat::RED . TextFormat::BOLD . "Du kannst dem Team $teamname nicht Joinen, Begründung: Das Team ist voll");
            return false;
        }
        ...
        if ($this->getState() === self::WAITING || $this->getState() === self::STARTING || $this->getState() === self::IDLE) {
            ...
            if (count($this->getPlayers()) < $this->getMinPlayers()) Server::getInstance()->broadcastMessage(TextFormat::RED . TextFormat::BOLD . "Das Spiel " . $gamename . " braucht Spieler!", Server::getInstance()->getDefaultLevel()->getPlayers());
            elseif (count($this->getPlayers()) < $this->getMaxPlayers()) Server::getInstance()->broadcastMessage(TextFormat::DARK_GRAY . TextFormat::BOLD . "Das Spiel " . $gamename . " ist nicht Voll, du kannst noch Joinen!", Server::getInstance()->getDefaultLevel()->getPlayers());
            $this->bossbar->setPercentage(1)->setTitle($gamename . " " . strval($this->getMinPlayers() - count($this->getPlayers())) . " Spieler fehlt noch");
        ...
        $player->sendMessage($team->getColor() . TextFormat::BOLD . "Du bist im Team " . $team->getName());
    ...
    public function sendTimer(Game $game)
    {
        if (!$this->forcedStart && count($this->getPlayers()) < $this->getMinPlayers()) {
            Server::getInstance()->broadcastTitle(TextFormat::DARK_RED . "Zu wenig Spieler", "für " . $game->getPrefix() . ', ' . $this->getMinPlayers() . ' players are needed!');
            $this->resetTimer();
            return;
        }
        $this->bossbar->setTitle('BedWars ' . str_replace('[', '', str_replace(']', '', $game->getPrefix())) . ' starts in ' . $this->timer . ' seconds')->setPercentage($this->timer / self::TIMER_MAX);
        $this->timer--;
        if ($this->timer < 0) {
            $this->resetTimer();
            $this->startArena();
